        ...
        while (lines.hasNext()) {
            ...
            while (currentLine.startsWith("#")) {
                ...
                if (currentLine.startsWith("#EXTVLCOPT:")) {
                    val opt = currentLine.drop(11).trim()
                    val pair = opt.split("=", limit = 2)
                    if (pair.size == 2) {
                        val key = pair[0].trim().lowercase()
                        val value = pair[1].trim()
                        if (key == "http-user-agent" || key == "user-agent") vlcOpts["User-Agent"] = value
                        if (key == "http-referrer" || key == "http-referer" || key == "referrer") vlcOpts["Referer"] = value
                    }
                }
                if (lines.hasNext()) {
                    currentLine = lines.next()
            ...
            val metadata = buildMap {
                ...
                for (match in matches) {
                    ...
                    val value = (match.groups[2]?.value ?: match.groups[3]?.value)?.ifBlank { null } ?: continue
            ...
            // Integrar headers na URL no formato Kodi (|Header=Value)
            var finalUrl = currentLine
            if (vlcOpts.isNotEmpty()) {
                val options = vlcOpts.map { "${it.key}=${it.value}" }.joinToString("&")
                finalUrl = if (finalUrl.contains("|")) "$finalUrl&$options" else "$finalUrl|$options"
            }
