...
class SettingViewModel @Inject constructor(
    ...
    fun addPlaylist(title: String, url: String) {
        viewModelScope.launch {
            try {
                // Validation could be added here if needed, e.g. checking if URL is empty or valid format
                if (title.isEmpty() || url.isEmpty()) {
                    messager.emit("Título e URL são obrigatórios")
                    return@launch
                }
                SubscriptionWorker.m3u(workManager, title, url)
                messager.emit(SettingMessage.Enqueued)
            } catch (e: Exception) {
                messager.emit(e.message ?: "Erro ao adicionar lista")
            }
        }
    }

    fun updatePlaylistTitle(url: String, title: String) {
        viewModelScope.launch {
            playlistRepository.onUpdatePlaylistTitle(url, title)
        }
    }

    fun updatePlaylistUrl(id: Int, url: String) {
        viewModelScope.launch {
            playlistRepository.onUpdatePlaylistUrl(id, url)
        }
    }
    ...
    fun applyColor(
        ...
        viewModelScope.launch {
            settings.update(PreferencesKeys.DARK_MODE, isDark)
            if (prev != null) {
