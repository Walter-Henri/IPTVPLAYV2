...
interface PlaylistDao {
    ...
    @Query("UPDATE playlists SET url = :newUrl WHERE id = :id")
    suspend fun updateUrl(id: Int, newUrl: String)
    ...
    suspend fun updateChannelsPlaylistUrl(oldUrl: String, newUrl: String)

    @Transaction
    suspend fun updateUrl(oldUrl: String, newUrl: String) {
        updatePlaylistUrl(oldUrl, newUrl)
        updateChannelsPlaylistUrl(oldUrl, newUrl)
    }

    @Query("UPDATE playlists SET active = 0")
    suspend fun deselectAll()

    @Query("UPDATE playlists SET active = 1 WHERE url = :url")
    suspend fun setActive(url: String)
    
    @Transaction
    suspend fun updateActive(url: String) {
        deselectAll()
        setActive(url)
    }
