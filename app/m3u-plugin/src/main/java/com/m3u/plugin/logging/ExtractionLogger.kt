package com.m3u.plugin.logging
...
class ExtractionLogger(private val context: Context) {
    ...
    suspend fun logAttempt(
        ...
        val logLevel = if (success) com.m3u.plugin.util.LogManager.LogLevel.INFO else com.m3u.plugin.util.LogManager.LogLevel.WARN
        com.m3u.plugin.util.LogManager.log(
            message = "[$method] $statusIcon $channelName (${duration}ms)",
            level = logLevel,
            tag = "EXTRACTION"
        )
        if (!success && error != null) {
            com.m3u.plugin.util.LogManager.warn("  → Motivo: $error", "EXTRACTION")
        }
    }
    ...
    suspend fun logChannel(
        ...
        val uiLevel = if (success) com.m3u.plugin.util.LogManager.LogLevel.INFO else com.m3u.plugin.util.LogManager.LogLevel.ERROR
        com.m3u.plugin.util.LogManager.log(
            message = "$status: $name",
            level = uiLevel,
            tag = "FINAL"
